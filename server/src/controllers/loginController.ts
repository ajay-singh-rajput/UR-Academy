...
export const verifyUserOTP = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (user.verifyCode !== req.body.otp) return next(new ErrorHandler('Wrong OTP', 403));
...
export const verifyUserLink = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user) return next(new ErrorHandler("User not found", 404));
    if (user.verifyCode !== req.params.code) return next(new ErrorHandler('Invalid Link', 404));
    user.isVerified = true;
    user.verifyCode = '';
    user.save();
...
export const loginUser = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!user || !user2) return next(new ErrorHandler("User not found", 404))
    const isMatch = user.comparePassword(req.body.password);
    if (!isMatch) return next(new ErrorHandler("Wrong Credentials", 401));
    if (!user.isVerified) {
        const vCode = `${Math.floor(Math.random() * 999) + 1000}`;
        user2.verifyCode = vCode
        await user2.save();
        const url = `${req.protocol}://${req.get('host')}/user/verify-link/${user._id}/${vCode}`;
        const msg = `click blow link to verify your email address`
        sendMail(req, res, next, url, vCode, msg);
        return next(new ErrorHandler("Please verify your email address to get login", 403));
    }
